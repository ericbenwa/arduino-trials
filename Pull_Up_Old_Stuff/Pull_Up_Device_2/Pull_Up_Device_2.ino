...
long timeOfLastPullUp;          //wehn my last pullup started
long elapsedTime;
...
void pullUp(){                             // actions performed for each pull up
  ...
  active = 1;
  timeBetween = currentTime - timeOfLastPullUp;     
  timeOfLastPullUp = currentTime;  
...
//    timeBetween = millis(); //480000000        // not set up for my need, this counter start as soon as the app starts, regardless if the time status is 1
//  }
...
void sendData(){
  ...
  if (active == 1) {
    long currentTime = millis();
    elapsedTime = currentTime - timeOfLastPullUp;
 
    if (elapsedTime > 30000){              // after 30 seconds from last pull up lets upload the data

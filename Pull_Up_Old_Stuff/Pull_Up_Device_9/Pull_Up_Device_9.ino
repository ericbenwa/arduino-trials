...
void loop(){
  ...
  if (distance < 20 && distance > 0 && active == 0) {
    pullUp();
    active = 1;
...
/* Doesn't work with NewPing library
...
void sendData(){                          // actions performed when we want to upload data to the web
  ...
  if (activePerson == 1) {
    ...
    if (elapsedTimeSinceLastPullUp > 7000){              // after 7 seconds from last pull up lets upload the data
      connectWifi();
...
void connectWifi() {
  while (status != WL_CONNECTED) {
    ...
    Serial.println("sent the id and password");
    delay(10000);
  }
  printWifiStatus();
}
